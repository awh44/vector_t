...
int main(int argc, char *argv[])
{
	...
	if (argv[1][0] == 'i')
	{
		int_vector_t *v = int_vector_initialize();
		int i;
		for (i = 0; i < 20; i++)
		{
		...
		int_vector_t *vcopy = int_vector_initialize();
		int_vector_copy(vcopy, v);
		printf("v->capacity = %zu, vcopy->capacity = %zu\n", int_vector_get_capacity(v), int_vector_get_capacity(vcopy));
		...
		int_vector_t *v_array = int_vector_initialize();
	...
	{
		char_vector_t *c = char_vector_initialize(),
		              *c_copy = char_vector_initialize(),
		              *c_goodbye = char_vector_initialize();
		...
		printf("%s\n", char_vector_c_array(c));
		printf("%s\n", char_vector_c_array(c_copy));
		printf("%.*s\n", char_vector_size(c_goodbye), char_vector_c_array(c_goodbye));
